...
use plugin\owladmin\app\trait\MakeTrait;
        ...
        try {
            ...
            if ($needs->contains('need_create_table')) {
                if (DB::schema()->hasTable($record->table_name)) { // webman
                    abort(Response::HTTP_BAD_REQUEST, "Table [{$record->table_name}] already exists!");
                }

                if ($migratePath) {
                    $output = runCommand('migrate:run --path=' . rtrim($migratePath, '.php')); //  webman
                } else {
                    $output = runCommand('migrate:run'); // webman
                }
