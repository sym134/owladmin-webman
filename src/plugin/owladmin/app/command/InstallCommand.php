...
class InstallCommand extends BaseCommand
{
    ...
    {
        $this->directory = config('plugin.owladmin.admin.directory');
    }
...
]", "'/' . config('plugin.owladmin.admin.route.prefix')", $contents);
...
use Illuminate\Support\Facades\Route;', 'use Webman\Route;', $contents);
    ...
    public function createHomeController(): void
    {
        ...
        $this->filePut($path, $file_name, str_replace('{{Namespace}}', config('plugin.owladmin.admin.route.namespace'), $contents));
    ...
    {
        $base = str_replace('\\controller', '\\', config('plugin.jizhi.owladmin.admin.route.namespace'));
