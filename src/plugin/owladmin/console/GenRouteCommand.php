...
use plugin\owladmin\app\Admin;use Illuminate\Routing\Router;use Illuminate\Support\Facades\Route;
...
EOF;
        ...
        AdminApiService::make()->query()->where('enabled', 1)->get()->map(function ($item) use (&$routes) {
            ...
            $routes .= <<<EOF
    // {$item->title}
    \$router->{$item->method}('{$_route}', [\plugin\owladmin\controller\AdminApiController::class, 'index']);
